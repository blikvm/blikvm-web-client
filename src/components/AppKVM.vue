...
<script setup>
  ...
  const { turnOnMic, turnOffMic } = useMicrophone(device); //error
  ...
  } = useVideo(device, streamElementRef);
  ...
  // Watch for videoMode changes
  watch(
    () => device.value.video.videoMode,
    (newVal, oldVal) => {
      console.log('videoMode changed:', oldVal, '→', newVal);
      if (wasNoSignal.value === true) {
        return;
      }
      if (newVal === 'h264') {
